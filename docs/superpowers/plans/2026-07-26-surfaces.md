...
Run: `npx vitest run test/review-queue.test.ts`
...
Run: `npx vitest run test/eval-scale.test.ts`
...
Confirm `package.json` has no `stockfish` entry afterward, and that `package-lock.json` was updated by the uninstall.
...
Run: `npx playwright test test/browser/engine-asset.spec.ts`
Expected: PASS.
...
```bash
git add public/engine package.json package-lock.json test/browser/engine-asset.spec.ts
