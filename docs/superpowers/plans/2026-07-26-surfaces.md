...
- The real unit-test gate is `npm run test:emulators`. Plain `npm test` cannot pass, because `test/auth.test.ts` and `test/rules.test.ts` require the Firebase emulators. The browser gate is `npm run test:browser`.
...
```bash
npm install --save-dev stockfish@18.0.8
