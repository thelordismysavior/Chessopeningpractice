...
export async function renderSources(navigate: Navigate, email: string | null): Promise<void> {
  ...
  app.querySelector('.sources-page > .eyebrow')?.replaceChildren('Source notes');
  app.querySelector('.sources-page > h1')?.replaceChildren('Keep the reason close.');
  app.querySelector('.sources-page > .lede')?.replaceChildren('Each line has a source so you can review the position, not just the notation.');
