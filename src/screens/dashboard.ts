...
function levelButton(course: Course, level: LevelKey, progress: CourseProgress): string {
  ...
  return `<button class="lesson-row" data-course="${course.id}" data-level="${level}"${index > progress.unlockedLevel ? ' disabled' : ''}><span class="lesson-index">${String(index + 1).padStart(2, '0')}</span><span><strong>${levelNames[level]}</strong><small>${detail}</small></span>${lineMeter(course, level, progress)}<span class="lesson-arrow" aria-hidden="true">${complete ? '&#10003;' : '-&gt;'}</span></button>`;
}
  ...
  for (const course of COURSES) {
    ...
    for (const level of LEVELS) {
      for (const variation of course.lessons[level].variations) {
        const dueIds = duePositionIds(progress.positions, variation.positions.map((position) => position.id));
        if (dueIds.length) return { course, level, progress, variation, duePositionIds: dueIds };
      }
    }
  }
  for (const course of COURSES) {
