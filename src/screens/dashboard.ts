...
export async function renderDashboard(navigate: Navigate, email: string | null): Promise<void> {
  ...
  try {
    ...
    app.innerHTML = `<main class="app-shell">${topbarMarkup({ email, wordmark: true, links: [{ id: 'browse', label: 'Browse lines' }, { id: 'sources', label: 'Sources' }, { id: 'settings', label: 'Settings' }] })}<section class="dashboard-intro"><div><p class="eyebrow">White + Black repertoire</p><h1>Make the first move<br><em>automatic.</em></h1><p class="lede">Four focused systems. Three levels each. Practice the line until it feels like your own.</p></div>${masteryMarkup}</section><section class="course-grid">${COURSES.map((course) => {
